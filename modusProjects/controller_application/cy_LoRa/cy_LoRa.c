...
// Returns:	uint8_t - The value that was read
uint8_t readRegister(uint8_t address) { return singleTransfer((address & 0x7f), 0); }
...
// Returns:	void
void writeRegister(uint8_t address, uint8_t value) { singleTransfer((address | 0x80), value); }
...
// Params:	void
// Returns:	void
void sleep(void) { writeRegister(REG_OP_MODE, MODE_LONG_RANGE_MODE | MODE_SLEEP); }
...
uint8_t LoRa_Init(void) {
	...
	// Set output power to 20 dBm, the maximum
...
void onTxDone(void (*callback)(void)) {
	...
	if(callback && !myLoRa.interruptEnabled) {
		static const cy_stc_sysint_t intrCfg = {/*.intrSrc =*/LoRa_DIO_0_IRQ,
												/*.intrPriority =*/3UL};
...
float packetSnr(void) { return ((int8_t)readRegister(REG_PKT_SNR_VALUE)) * 0.25; }
...
uint8_t available(void) { return (readRegister(REG_RX_NB_BYTES) - myLoRa.packetIndex); }
...
// Params:	void
// Returns:	void
void enableCrc(void) { writeRegister(REG_MODEM_CONFIG_2, readRegister(REG_MODEM_CONFIG_2) | 0x04); }

// Function to disable CRC
// Params:	void
// Returns:	void
void disableCrc() { writeRegister(REG_MODEM_CONFIG_2, readRegister(REG_MODEM_CONFIG_2) & 0xfb); }
